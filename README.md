...
*   **安全验证**：支持 Cloudflare Turnstile、Google reCAPTCHA 以及自定义 Q&A 提问验证 [1]。
*   **消息过滤**：可设置屏蔽词库，并支持根据消息类型（转发、媒体、语音、贴纸等）开启或关闭转发 [1]。
*   **话题聚合**：自动在管理群创建话题，包含“🔔 未读消息”聚合话题和“🚫 黑名单”管理话题 [1]。
*   **用户资料卡**：管理员可实时查看用户 ID、加入时间，并支持添加备注信息 [1]。
*   **频率限制**：内置幂等校验与全局/用户限流，有效防止垃圾信息攻击 [1]。
...
*   **用户端**：
    *   `/start`：开始验证或进入对话。
    *   `/del`：引用消息后发送，可执行双向撤回 [1]。
*   **管理员端**：
    *   `/start`：打开管理控制面板。
    *   `/help`：查看管理命令说明。
    *   `/reset <user_id>`：强制重置特定用户的验证状态 [1]。
