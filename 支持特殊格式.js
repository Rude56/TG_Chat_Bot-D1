...
async function handleAdminInput(id, msg, state, env) {
  // 内部辅助函数：将 Telegram 的格式信息转换为 HTML 标签
  const convertToHTML = (text, entities) => {
    if (!entities || entities.length === 0) return text;
    let result = "";
    let lastOffset = 0;
    const sortedEntities = [...entities].sort((a, b) => a.offset - b.offset);
    for (const entity of sortedEntities) {
      result += text.substring(lastOffset, entity.offset);
      ...
      const tag = tagMap[entity.type];
      if (tag) {
        result += `<${tag}>${part}</${tag}>`;
      } else if (entity.type === 'text_link') {
        result += `<a href="${entity.url}">${part}</a>`;
      } else {
        result += part;
      }
      lastOffset = entity.offset + entity.length;
    }
    return result + text.substring(lastOffset);
  };

  const txt = msg.text || "";
  // 获取带格式的文本（如果是图片说明则用 caption，否则用 text）
  const formattedText = msg.entities ? convertToHTML(txt, msg.entities) : (msg.caption_entities ? convertToHTML(msg.caption || "", msg.caption_entities) : txt);
  ...
  let k = state.key, val = formattedText; // 默认使用带格式的文本
  const originalKey = k.replace("_add", "");
  ...
  try {
    ...
    if (k === "welcome_msg") {
      if (msg.photo || msg.video || msg.animation) {
        ...
        val = JSON.stringify({ type: type, file_id: fileId, caption: formattedText });
      } else {
        val = formattedText;
      }
    } 
    // 2. 处理时间设置（保持纯数字逻辑，不解析格式）
    else if (k === "sleep_start" || k === "sleep_end") {
      let cleanVal = txt.replace(/:/g, ":").trim();
      if (/^\d:\d{2}$/.test(cleanVal)) cleanVal = "0" + cleanVal;
      if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(cleanVal)) {
        return api(env.BOT_TOKEN, "sendMessage", {
          chat_id: id,
          text: "❌ <b>时间格式错误</b>\n请输入 24 小时制时间，例如 <code>08:00</code>。",
          parse_mode: "HTML"
        });
      }
      val = cleanVal;
    }
    // 3. 处理添加类操作（自动回复、屏蔽词、管理员）
      ...
      if (k === "ar") {
        const [kk, rr] = txt.split("==="); // 关键词建议不带格式，所以用 txt
        if (kk && rr) {
          // 这里让回复内容支持格式
          const formattedResponse = convertToHTML(rr, msg.entities ? msg.entities.filter(e => e.offset > txt.indexOf("===")) : []);
          arr.push({ keywords: kk, response: formattedResponse, id: Date.now() });
        }
        else return api(env.BOT_TOKEN, "sendMessage", { chat_id: id, text: "❌ 格式错误,请使用:关键词===回复内容" });
      } else {
        if (k === "auth" && !/^\d+$/.test(txt.trim())) {
          return api(env.BOT_TOKEN, "sendMessage", { chat_id: id, text: "❌ 请输入纯数字 ID" });
        }
        arr.push(txt.trim());
      }
  ...
  let displayVal = (val.startsWith("{") && originalKey === "welcome_msg") ? "[媒体配置]" : val.substring(0, 100);
