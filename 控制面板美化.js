  ...
  try {
    if (!type || type === "menu") {
      ...
      if (["ar", "kw", "auth"].includes(key)) {
        ...
        const label = listLabels[key] || key;
        // 使用更美观的标题和 Emoji
      ...
      if (key === "sleep") {
        ...
        return render(`🌙 <b>就寝时间设置</b>\n状态: ${on ? "✅ 自动启用" : "❌ 已关闭"}\n区间: <code>${start}</code> - <code>${end}</code>\n提示语: ${escapeHTML(msgText)}`, {
          inline_keyboard: [
            [{ text: `模式切换`, callback_data: `config:toggle:enable_sleep_mode:${!on}` }],
    ...
    if (type === "edit" || type === "add") {
      ...
      const label = keyLabels[key] || key; // 如果没有映射则显示原 key
      let promptText = `请输入新的 <b>${label}</b>\n退出点 /cancel`;
  
      // 针对特定功能的特殊提示保持不变，但可以进一步美化 [1]
