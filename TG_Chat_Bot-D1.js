/**  
* Telegram Bot Worker v3.85 (完整增强版)  
* 基于 v3.70+ 架构，集成引用同步、双向撤回、编辑同步  
*  
* 新增功能:  
* 1. 引用同步: 用户回复机器人消息，管理端显示引用；管理员回复消息，用户端显示引用。  
* 2. 撤回指令: 回复消息发送 /del，可同时删除用户端和管理端的消息。  
* 3. 编辑同步: 修改消息内容会自动同步到另一端。  
* 4. 菜单更新: 用户和管理员均可使用 /del 菜单。
*/  
  ...
  async fetch(req, env, ctx) {  
    ...
    try {  
      if (req.method === "GET") {  
        ...
        if (url.pathname === "/") return new Response("Bot v3.85 (Sync+Recall+Edit)", { status: 200 });  
      }  
...
async function relayToTopic(msg, u, env, ctx) {  
  ...
  try {  
    // 使用 copyMessage 以支持 reply_to_message_id  
    const extra = {};  
...
// --- 18. Commands (已更新: 用户可见 /del) ---  
async function registerCommands(env) {  
  try {  
    // 1. 设置默认用户的菜单 (普通用户: Start + Del)
...
async function handleAdminReply(msg, env) {  
  ...
  try {  
    const sent = await api(env.BOT_TOKEN, "copyMessage", {  
      ...
      message_id: msg.message_id,  
      reply_to_message_id: replyToIdInUser // 关联引用  
    });  
...
async function handleAdminConfig(cid, mid, type, key, val, env) {  
  ...
  try {  
    if (!type || type === "menu") {  
      ...
      if (key === "busy") {  
        const on = await getBool("busy_mode", env), msgText = await getCfg("busy_msg", env);  
