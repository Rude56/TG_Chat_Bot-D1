...
async function handleAdminConfig(cid, mid, type, key, val, env) {
  ...
  try {
    if (!type || type === "menu") {
      // --- 主菜单 ---
      if (!key) return render("<b>⚙️ 系统管理中心</b>\n请选择下方功能模块进行配置：", {
        inline_keyboard: [
          [{ text: "🛡️ 验证设置", callback_data: "config:menu:base" }, { text: "🤖 自动回复", callback_data: "config:menu:ar" }],
          [{ text: "🚫 屏蔽词库", callback_data: "config:menu:kw" }, { text: "🛠️ 过滤规则", callback_data: "config:menu:fl" }],
          [{ text: "👮 协管授权", callback_data: "config:menu:auth" }, { text: "🌙 就寝模式", callback_data: "config:menu:sleep" }],
        ]
      });
      ...
      if (key === "base") {
        const mode = await getCfg("captcha_mode", env),
          captchaOn = await getBool("enable_verify", env),
          qaOn = await getBool("enable_qa_verify", env);
        let modeText = "❌ 已关闭";
        if (captchaOn) modeText = mode === "recaptcha" ? "🔵 Google reCAPTCHA" : "🟠 Cloudflare Turnstile";

        return render(`<b>${labels.base}</b>\n\n<b>当前验证状态：</b>\n├ 外部验证：<code>${modeText}</code>\n└ 问题验证：<code>${qaOn ? "✅ 已开启" : "❌ 已关闭"}</code>`, {
          inline_keyboard: [
            [{ text: "👋 编辑欢迎语", callback_data: "config:edit:welcome_msg" }],
            [{ text: "❓ 修改问题", callback_data: "config:edit:verif_q" }, { text: "🔑 修改答案", callback_data: "config:edit:verif_a" }],
            [{ text: `🔄 切换验证模式`, callback_data: `config:rotate_mode` }],
            [{ text: `${qaOn ? "🔕 关闭" : "🔔 开启"}问题验证`, callback_data: `config:toggle:enable_qa_verify:${!qaOn}` }],
            [back]
          ]
        });
      }
      ...
      // --- 过滤规则 ---
      if (key === "fl") {
        return render(`<b>${labels.fl}</b>\n配置哪些类型的消息将被自动拦截或限制：`, await getFilterKB(env));
      }

      // --- 列表类 (自动回复, 屏蔽词, 管理员) ---
      if (["ar", "kw", "auth"].includes(key)) {
        return render(`<b>${labels[key]}</b>\n点击列表项可直接删除，点击下方按钮添加新项：`, await getListKB(key, env));
      }
      ...
      if (key === "sleep") {
        ...
        return render(`<b>${labels.sleep}</b>\n\n<b>当前配置：</b>\n├ 状态：${on ? "✅ 运行中" : "❌ 已休眠"}\n├ 时间：<code>${start}</code> - <code>${end}</code>\n└ 提示：<i>${escapeHTML(msgText)}</i>`, {
          inline_keyboard: [
            [{ text: `${on ? "🔴 停用" : "🟢 启用"}就寝模式`, callback_data: `config:toggle:enable_sleep_mode:${!on}` }],
            [{ text: "🌙 设定睡觉时间", callback_data: "config:edit:sleep_start" }, { text: "⏰ 设定起床时间", callback_data: "config:edit:sleep_end" }],
            [{ text: "✏️ 修改睡眠提示语", callback_data: "config:edit:sleep_msg" }],
            [back]
          ]
        });
      }
    ...
    if (type === "del") {
      ...
      return render(`<b>${labels[key]}</b>\n✅ 已移除该条目。`, await getListKB(key, env));
    }
    ...
    if (type === "edit" || type === "add") {
      ...
      let promptText = `<b>📝 正在修改：${label}</b>\n\n请直接发送新的内容给我。\n————————————————\n💡 提示：点击 /cancel 取消本次操作。`;

      if (key === "sleep_start" || key === "sleep_end") {
        promptText = `<b>⏰ 设定时间：${label}</b>\n\n请输入 24 小时制时间，例如：<code>23:30</code> 或 <code>07:00</code>。\n————————————————\n💡 点击 /cancel 退出修改。`;
      } else if (key === "ar" && type === "add") {
        promptText = `<b>🤖 添加自动回复</b>\n\n请使用以下格式发送：\n<code>关键词===回复内容</code>\n\n范例：<code>价格===请咨询官方客服</code>\n————————————————\n💡 点击 /cancel 退出添加。`;
      } else if (key === "welcome_msg") {
        promptText = `<b>👋 设置入群欢迎语</b>\n\n• 支持发送<b>文字、图片、视频或 GIF</b>\n• 文字支持占位符：<code>{name}</code>\n————————————————\n💡 直接发送媒体或文字即可，或点 /cancel 退出。`;
      }
    ...
    if (type === "rotate_mode") {
      ...
      if (isEnabled) {
        ...
        else { nextEnable = "false"; nextMode = currentMode; toast = "✅ 外部验证已关闭"; }
      }
      await setCfg("captcha_mode", nextMode, env);
      await setCfg("enable_verify", nextEnable, env);
      return render(`<b>基础安全更新</b>\n\n${toast}`, { inline_keyboard: [[{ text: "🔙 返回", callback_data: "config:menu:base" }]] });
    }
...
async function getFilterKB(env) {
  ...
  return {
    inline_keyboard: [
      [b("禁止转发内容", keys[0], vals[0])],
      [b("图片媒体", keys[1], vals[1]), b("语音音频", keys[2], vals[2])],
      [b("静态/动图贴纸", keys[3], vals[3]), b("超链接", keys[4], vals[4])],
      [b("频道消息", keys[5], vals[5]), b("纯文本", keys[6], vals[6])],
...
async function getListKB(type, env) {
  ...
  const btns = (Array.isArray(l) ? l : []).map(i => [{
    text: `🗑 删除: ${type === "ar" ? i.keywords : i}`,
    callback_data: `config:del:${type}:${i.id || i}`
  }]);
  btns.push([{ text: "➕ 添加新条目", callback_data: `config:add:${type}` }], [{ text: "🔙 返回主菜单", callback_data: "config:menu" }]);
...
async function handleAdminInput(id, msg, state, env) {
  ...
  const keyLabels = {
    ...
    "sleep_msg": "就寝提示语",
    "block_keywords": "屏蔽词库",
    "keyword_responses": "自动回复规则",
    "authorized_admins": "管理员授权"
  };
  ...
  try {
    ...
    if (k === "sleep_start" || k === "sleep_end") {
      ...
      if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(cleanVal)) {
        return api(env.BOT_TOKEN, "sendMessage", {
          chat_id: id,
          text: "⚠️ <b>时间格式不正确</b>\n\n请输入 <code>HH:mm</code> 格式 (例如 08:30)。",
          parse_mode: "HTML"
        });
      }
      val = cleanVal;
    ...
    if (k === "welcome_msg") {
      ...
      if (k === "ar") {
        ...
        else return api(env.BOT_TOKEN, "sendMessage", { chat_id: id, text: "⚠️ 格式错误，请使用 <code>关键词===回复内容</code>" , parse_mode: "HTML"});
      } else arr.push(txt);
    ...
    const displayVal = (val.startsWith("{") && (k === "welcome_msg" || k === "keyword_responses")) ? "🖼️ 媒体/结构化内容" : val.substring(0, 50);
    
    await api(env.BOT_TOKEN, "sendMessage", {
      chat_id: id,
      text: `✅ <b>保存成功</b>\n\n项目：<code>${keyLabels[k] || k}</code>\n内容：<i>${escapeHTML(displayVal)}</i>`,
      parse_mode: "HTML"
    });

    return handleAdminConfig(id, null, "menu", null, null, env);
  } catch (e) {
    api(env.BOT_TOKEN, "sendMessage", { chat_id: id, text: `❌ 保存失败: ${e.message}` }).catch(() => { });
  }
}
// --- 22. 表态同步 ---
