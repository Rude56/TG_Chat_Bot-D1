...
async function dbInit(env) {
  ...
  ait env.TG_BOT_DB.prepare(`CREATE TABLE IF NOT EXISTS users (
    user_id TEXT PRIMARY KEY, 
    topic_id TEXT, 
    user_state INTEGER DEFAULT 0, -- 0:未验证, 1:已验证, 2:已封禁
...
async function handleUpdate(update, env, ctx) {
  ...
  if (msg.chat.type === "private") {
      ...
      // 不再校验 isAuthAdmin，只要在管理群且有话题 ID 就转发 [1]
...
async function handlePrivate(msg, env, ctx) {
  ...
  const u = await getUser(id, env); // 状态自动默认为 0
  ...
  // 2. 未验证判定
  if (u.user_state === 0) {
      return sendStart(id, msg, env); // 触发 Turnstile 验证
  }
...
async function handleVerifiedMsg(msg, u, env, ctx) {
  ...
  if (await getBool("enable_sleep_mode", env)) {
    ...
    if (checkSleepTime(start, end)) { // 自定义时间判定
        // 只表态，不转发，不发消息
        return api(env.BOT_TOKEN, "setMessageReaction", {
            chat_id: id,
